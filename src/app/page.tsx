...
export default function Home() {
  ...
  const [authorizingState, setAuthorizingState] = useState<'NONE' | 'AUTHORIZING' | 'AUTHORIZED' | 'ERROR' | 'REVOKED'>('NONE')
  ...
  return (
    ...
    <div className='w-full h-screen flex justify-center items-center bg-[#3d5e58]'>
      <div className="bg-white rounded-lg shadow p-12 space-y-6 max-w-lg -full">
        ...
        {!authResponse && authorizingState !== 'REVOKED' &&  (<><p>This service is a propotype for token gating content within Mona virtual spaces. Currently, we use your IP address to assign authorized avatars, but in the future would love to use your wallet address so we can validate ownership of specific NFTs in targeted collections.</p>
        ...
        <div className='text-sm'>{`Authorized avatars are: ${authResponse.avatarIds.join(", ")}`}
        </div>
      </div>}
        {authorizingState === 'REVOKED' && <div className='mt-4'>
        <div className='text-lg font-bold'>IP Revoked!</div>
        </div>}
      ...
      <button
        ...
        >{authorizingState === 'NONE' || authorizingState === 'REVOKED' ? 'Authorize IP' : authorizingState === 'AUTHORIZING' ? 'Authrizing...' : authorizingState == 'AUTHORIZED' ? 'Authorized' : 'Error'}
      </button> 
      <button
